...
First of all, rendering untrusted HTML and CSS files can lead to [security
problems](first_steps.md#security). Please be sure to carefully follow the different proposed
...
such as [Flask-WeasyPrint](https://github.com/Kozea/Flask-WeasyPrint) (for [Flask](https://flask.palletsprojects.com/)) or [Django-WeasyPrint](https://github.com/fdemmer/django-weasyprint) (for [Django](https://www.djangoproject.com/))
...
such as page numbers, headers, etc. Read more about the [page](https://developer.mozilla.org/en-US/docs/Web/CSS/@page) at-rule.
...
`pdf_variant` Python parameter of [`HTML.write_pdf`](api_reference.md#weasyprint.HTML.write_pdf "weasyprint.HTML.write_pdf").
...
[the stylesheet set by the –pdf-forms option](https://github.com/Kozea/WeasyPrint/blob/main/weasyprint/css/html5_ua_form.css)
...
[`weasyprint.Attachment`](api_reference.md#weasyprint.Attachment "weasyprint.Attachment") class.
...
but it’s not the main culprit here. [Optimization is not the main goal of
WeasyPrint](going_further.md#why-python) and it may lead to unbearable long rendering times.

First of all: WeasyPrint’s performance gets generally better with time. You can
check [WeasyPerf](https://kozea.github.io/WeasyPerf/) to compare time and memory needed across versions.
...
- Optimizing images and fonts can reduce the PDF size, but increase the
  ...
  multiple times. See [Cache and Optimize Images](#cache-and-optimize-images).

## Show Log Messages

Most errors (unsupported CSS property, missing image…) are not fatal and will
not prevent a document from being rendered. WeasyPrint uses the [`logging`](https://docs.python.org/3/library/logging.md#module-logging "(in Python v3.14)")
...
See the documentation of the [`logging`](https://docs.python.org/3/library/logging.md#module-logging "(in Python v3.14)") module for details.
