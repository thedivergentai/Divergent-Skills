...
Source: https://doc.courtbouillon.org/weasyprint/stable/going_further.html
...
very dynamic way. But paged media layout is actually included in [CSS2](https://www.w3.org/TR/1998/REC-CSS2-19980512/), which
...
basic features. That’s why projects such as [wkhtmltopdf](https://wkhtmltopdf.org/) and [PagedJS](https://www.pagedjs.org/) have been
...
media. [Prince](https://www.princexml.com/), [Antennahouse](https://www.antennahouse.com/) or [Typeset.sh](https://typeset.sh/) created original renderers
...
create such a big project from scratch? Here is what [Simon Sapin](https://exyr.org/) wrote
...
following [**the Zen of Python**](https://peps.python.org/pep-0020/) helped a lot to keep our sanity (both in our
...
feel free to [ask](support.md#support)!

Much [like in web browsers](https://www.html5rocks.com/en/tutorials/internals/howbrowserswork/#The_main_flow), the rendering of a document in WeasyPrint goes
like this:
...
Not much to see here. The [`HTML`](api_reference.md#weasyprint.HTML "weasyprint.HTML") class handles step 1 and
...
As with HTML, CSS stylesheets are parsed in the [`CSS`](api_reference.md#weasyprint.CSS "weasyprint.CSS") class
with an external library, [tinycss2](https://pypi.python.org/pypi/tinycss2).
...
- Selectors are pre-compiled with [cssselect2](https://pypi.python.org/pypi/cssselect2).

### The Cascade

After that and still in the `css` package, the [cascade](https://www.w3.org/TR/CSS21/cascade.md)
...
[origin](api_reference.md#stylesheet-origins), `!important` markers, selector
...
pseudo-element is a string for eg. `::first-line` selectors, or [`None`](https://docs.python.org/3/library/constants.md#None "(in Python v3.14)")
...
The [visual formatting model](https://www.w3.org/TR/CSS21/visuren.md) explains how *elements* (from the ElementTree
...
in Python, in the [user agent stylesheet](https://github.com/Kozea/WeasyPrint/blob/main/weasyprint/css/html5_ua.css).
...
According to the [box model](https://www.w3.org/TR/CSS21/box.md), each box has rectangular margin, border,
padding and content areas:

While `box.style` contains computed values, the [used values](https://www.w3.org/TR/CSS21/cascade.md#used-value) are set as
...
content box) from the top-left corner of the page.[[1]](#id2)
...
These are the coordinates *if* no [CSS transform](https://www.w3.org/TR/css-transforms-1/) applies.
...
[stacking rules](https://www.w3.org/TR/CSS21/zindex.md) such as the `z-index` property. The result is a tree of
*stacking contexts*.
