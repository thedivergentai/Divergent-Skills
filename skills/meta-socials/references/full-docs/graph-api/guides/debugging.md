...
Debug Param Value | What Will Be Returned || all | All available debug messages. |
...
Field | Datatype | Description || message | String | The message. |
...
When [reporting a bug](/bugs/) in the Graph API, we include some additional request headers to send with your bug report to help us pinpoint and reproduce your issue. These request headers are `X-FB-Debug`, `x-fb-rev`, and `X-FB-Trace-ID`.
