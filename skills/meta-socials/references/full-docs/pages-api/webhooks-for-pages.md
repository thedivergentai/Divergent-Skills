# Webhooks - Facebook Pages API

Source: https://developers.facebook.com/docs/pages-api/webhooks-for-pages

# Webhooks for Pages

Webhooks for [Pages](pages.md) can send you real-time notifications of changes to your Pages. For example, you can receive real-time updates whenever users post to your feed, comment on a post, or like your posts.
...
Follow our [Getting Started guide](graph-api/webhooks/getting-started.md) to create your endpoint and configure the Webhooks product. During configuration, make sure to choose the **Page** object and subscribe to one or more of the Pages fields below.

Field | Description || `feed` | Notifies you when an Page's feed has changed; posts, reactions, shares, etc. |
...
- A Page access token requested from a person who can perform the
  ...
  `MODERATE`
  task](pages/overview.md#tasks) on the Page being queried
- The
...
- A Page access token requested from a person who can perform the [`MESSAGING` task](pages/overview.md#tasks) on the Page being queried
...
Use the `post_id` from the notification to [comment on that Page post](pages/publishing.md#comment_on_post).
